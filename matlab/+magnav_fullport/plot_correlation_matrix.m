...
% TODO(Julia->MATLAB): plot_png::String = "correlation_matrix.png")
    ...
    Nf = length(features)
    (Nf_min,Nf_max) = (2,5)
% TODO(Julia->MATLAB): assert Nf >= Nf_min "number of features = $Nf < $Nf_min"
% TODO(Julia->MATLAB): assert Nf <= Nf_max "number of features = $Nf > $Nf_max"

    p_ = []
% TODO(Julia->MATLAB): for j = 2:Nf, i = 1:Nf-1
        if j > i
...
% TODO(Julia->MATLAB): push!(p_,scatter(downsample(x_,Nmax),downsample(y_,Nmax),
% TODO(Julia->MATLAB): lab=false,dpi=dpi,mc=:black,ms=1,
