...
function out = gif_animation_m3(TL_perm, TL_induced, TL_eddy, TL_aircraft, B_unit, y_nn, y, y_hat, xyz, filt_lat, filt_lon, varargin)
    out = [];
% TODO(Julia->MATLAB): TL_aircraft::AbstractMatrix, B_unit::AbstractMatrix, y_nn::AbstractMatrix,
% TODO(Julia->MATLAB): y::Vector, y_hat::Vector, xyz::XYZ,
% TODO(Julia->MATLAB): filt_lat::Vector = [],
% TODO(Julia->MATLAB): filt_lon::Vector = [];
                          ind              = trues(xyz.traj.N),
% TODO(Julia->MATLAB): tt_lim::Tuple    = (0, (xyz.traj(ind).N-1)*xyz.traj.dt/60),
% TODO(Julia->MATLAB): skip_every::Int  = 5,
% TODO(Julia->MATLAB): save_plot::Bool  = false,
% TODO(Julia->MATLAB): mag_gif::String  = "comp_xai.gif")
...
% TODO(Julia->MATLAB): % most of the 3D vector may be in the "down" direction, which makes
...
% TODO(Julia->MATLAB): for i in i_start:skip_every:i_end
        p1 = plot(layout=l, size=(800,500), margin=4*mm)

        % move a vertical line across the magnetic field data
% TODO(Julia->MATLAB): plot!(p1(1),xlab="time [min]",ylab=" magnetic field [nT]",
% TODO(Julia->MATLAB): xlim=tt_lim,legend=:bottomleft)
% TODO(Julia->MATLAB): plot!(p1(1),tt,y      ,lab="true compensation"   ,lc=:gray, ls=:dash)
% TODO(Julia->MATLAB): plot!(p1(1),tt,y_hat  ,lab="model 3 compensation",lc=:black)
% TODO(Julia->MATLAB): plot!(p1(1),tt,TL_comp,lab="TL component"        ,lc=:blue)
% TODO(Julia->MATLAB): plot!(p1(1),tt,NN_comp,lab="NN component"        ,lc=:red)
% TODO(Julia->MATLAB): plot!(p1(1),[tt(i)]   ,lab=""                    ,lc=:black, lt=:vline)

        % draw compass plot for each field
% TODO(Julia->MATLAB): plot!(p1(2),xlab="east [nT]",ylab=" north [nT]",
% TODO(Julia->MATLAB): xlim=(-2500,2500),ylim=(-2500,2500),legend=:topright)
% TODO(Julia->MATLAB): plot!(p1(2),[0.0,aircraft_2D_TL(2,i)],[0.0,aircraft_2D_TL(1,i)],arrow=true,lab="TL",lc=:blue)
% TODO(Julia->MATLAB): plot!(p1(2),[0.0,aircraft_2D_NN(2,i)],[0.0,aircraft_2D_NN(1,i)],arrow=true,lab="NN",lc=:red)
% TODO(Julia->MATLAB): plot!(p1(2),[0.0,perm_field_2D( 2,i)],[0.0,perm_field_2D( 1,i)],arrow=true,lab="perm.")
% TODO(Julia->MATLAB): plot!(p1(2),[0.0,ind_field_2D(  2,i)],[0.0,ind_field_2D(  1,i)],arrow=true,lab="ind.")
% TODO(Julia->MATLAB): plot!(p1(2),[0.0,eddy_field_2D( 2,i)],[0.0,eddy_field_2D( 1,i)],arrow=true,lab="eddy")

        % plot airplane on map
% TODO(Julia->MATLAB): plot!(p1(3),xlab="longitude [deg]",ylab="latitude [deg]")
% TODO(Julia->MATLAB): plot!(p1(3),gps_lon(1:i) ,gps_lat(1:i) ,xlim=xlim,ylim=ylim,lab="GPS")
% TODO(Julia->MATLAB): show_ins  && (plot!(p1(3),ins_lon(1:i) ,ins_lat(1:i) ,xlim=xlim,ylim=ylim,lab="INS"))
% TODO(Julia->MATLAB): show_filt && (plot!(p1(3),filt_lon(1:i),filt_lat(1:i),xlim=xlim,ylim=ylim,lab="MagNav",xrotation=18))
% TODO(Julia->MATLAB): annotate!(gps_lon(i), gps_lat(i), Plots.text("✈", 20, rotation=dir(i)), subplot=3)
